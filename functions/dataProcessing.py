...
def processResults(path):
    ...
    for ind, line in enumerate(allLines[0:400]):
        data = line.split(" ")
        nlls[ind] = float(data[0])
        contexts[ind] = data[1]
        environments[ind] = int(data[2])
        taus[ind] = float(data[3])
        betas[ind] = float(data[4])
        lambdas[ind]= float(data[5])
        IDs[ind] = int(data[6])
        rnds[ind] = int(data[7])
        trials[ind] = int(data[8])
        chosens[ind] = int(data[9])
        contextOrders[ind] = int(data[10])
    return pd.DataFrame({"nll": nlls, 
                        "context": contexts,
...
def processResults2(path):
    ...
    chosens = [0]*res_len
    contextOrders = [0]*res_len
    for ind, line in enumerate(allLines[0:res_len]):
        data = line.split(" ")
        nlls[ind] = float(data[0])
        contexts[ind] = data[1]
        environments[ind] = int(data[2])
        ...
        loos[ind] = int(data[7])
        rnds[ind] = int(data[8])
        trials[ind] = int(data[9])
        chosens[ind] = int(data[10])
        contextOrders[ind] = int(data[11])
    return pd.DataFrame({"nll": nlls, 
                        "context": contexts,
                        "environment": environments,
                        "tau": taus,
...
def processResults4(path):
    ...
    for ind, line in enumerate(allLines[0:res_len]):
        ...
        nlls[ind] = float(data[0])
        contexts[ind] = data[1]
        taus[ind] = float(data[2])
        betas[ind] = float(data[3])
        lambdas[ind]= float(data[4])
        IDs[ind] = int(data[5])
        loos[ind] = int(data[6])
        rnds[ind] = int(data[7])
        trials[ind] = int(data[8])
...
def processAllResults(path, f=processResults5):
    filenames = [x for x in os.listdir(path) if x != ".git"]
    res = [0]*len(filenames)
    for ind, filename in enumerate(filenames):
        res[ind] = f(path + filename)
...
def readData(path="experimentData/full.csv"):
    ...
    for i in range(sampleSize):
        ...
        df = pd.concat([df, pd.DataFrame({"id": [Id]*n, 
                              "age": [age]*n, 
                              "gender": [gender]*n, 
...
def readData2(path="experimentData/full.csv"):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        fxn()
    rawdata = pd.read_csv(path, sep=",")
